...
export const EMPTY_PRICE = { purchaseRate: "", mrp: "", marginMode: "amount", margin: "", sellingRate: "" };
...
export default function PriceFields({ value, onChange, compact = false }) {
  ...
  return (
    <>
      <label className={lab}>Purchase Rate
        <input type="number" min="0" step="0.01" placeholder="₹ 50.00" value={v.purchaseRate} onChange={(e) => recompute({ purchaseRate: e.target.value })} className={`${inp} mt-1`} />
        {!compact && <span className={help}>Price paid to the supplier per unit.</span>}
      </label>
      <label className={lab}>MRP
        ...
        {!compact && <span className={help}>Maximum retail price printed on the pack.</span>}
      </label>
      <div className={lab}>
        Margin
