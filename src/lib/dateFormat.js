/**
 * Every pharmacy screen shows dates as DD/MM/YY — storage stays a real
 * Date/DateTime column everywhere; this is presentation only.
 */
export function fmtDDMMYY(value) {
  if (!value) return "—";
...
export function fmtDDMMYYYY(value) {
  if (!value) return "—";
  const d = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(d.getTime())) return "—";
  const dd = String(d.getDate()).padStart(2, "0");
