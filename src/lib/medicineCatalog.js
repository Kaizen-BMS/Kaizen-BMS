...
const medicineInputSchema = z.object({
  ...
  packSize: opt(60),
  contentUnit: opt(30),
  contentPerPack: z.coerce.number().int().min(1).max(1_000_000).optional(),
  unit: opt(30),
  purchaseUnit: opt(30),
...
function serializeMedicine(m) {
  ...
  return {
    ...
    contentUnit: m.content_unit,
    contentPerPack: m.content_per_pack,
    unit: m.unit,
...
function toRow(v, userId) {
  ...
  return {
    ...
    content_unit: v.contentUnit || null,
    content_per_pack: v.contentUnit ? v.contentPerPack ?? null : null,
    unit: v.unit || null,
