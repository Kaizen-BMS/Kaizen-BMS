...
## Simplify pass — Pharmacy / Prescription / Lab / Staff (2026-09-24)

One real-world concept looks like one simple concept. Migrations 047 (medicine `base_name`, wider `rack`) and 048 (lab panels, staff schedules/shift templates/history, profile duty columns).

- **Medicine identity**: `medicines.name` is the ONE display name everywhere ("Cap Betadine 500 mg"), composed server-side from type + bare name + strength (`src/lib/medicineName.js`, `composeMedicineName`); `base_name` keeps the bare name. Editing only re-composes (and renames matching `pharmacy_stock.medicine_name` rows) when name/type/strength actually change. Location is one free-text field (rack column, widened); Bin/Shelf boxes are gone from the UI (columns kept).
- **Autocomplete**: `src/lib/medicineSuggest.js` + `MedicineInput.jsx`; pharmacy route `/api/pharmacy/medicines/suggest` (`medicine:read`) and doctor route `/api/opd/medicine-suggest` (`prescription:create`). Searches name/salt/brand/composition/barcode, ranks prefix matches first, shows non-expired stock.
- **Pharmacy UI** = 3 business categories (Customers & Sales / Inventory / Suppliers & Purchase) + Reports; new tab files `InventoryTab.jsx`, `MedicinesTab.jsx`, `GrnTab.jsx`, `PurchaseOrdersTab.jsx`, `PurchaseHistoryTab.jsx`. PO = qty + medicine only (server defaults rate 0). GRN/Inventory use Qty · Medicine · Batch · MFD · Expiry · Purchase Rate · MRP · Margin (₹ or %) · Selling Price via `PriceFields.jsx` (`pharmacyPricing.js`); selling price is capped at MRP client-side AND rejected server-side (`selling_price_above_mrp`); GRN now stores `selling_rate`. `DateInput.jsx` types/shows DD/MM/YY (storage stays real dates).
- **Prescription**: `rxQuantity.js` — quantity = dose × times/day × days (OD/BD/TID/QID/HS/STAT computed; SOS/custom manual); override needs a reason (written into the dosage string). Consultation screen is two columns: current consultation left, `PatientHistorySidebar.jsx` right (same history API and its `consultation:read` gate).
- **Lab**: `/api/lab/order-catalog` groups what a doctor can order by laboratory (own + connected partner labs — names, sample, TAT, price, test sets only); `lab_panels`/`lab_panel_items` + `/api/lab/panels` are test sets (expanded to component tests, de-duplicated, for our own lab; a partner receives the set as one order because the external order carries one test name). `LabOrderPicker.jsx` is the doctor UI; sets are managed in Lab → Tests & prices (`LabPanels.jsx`).
- **Staff**: `staffSchedule.js` resolves plan = roster shift for the date, else weekly schedule (`staff_schedules`), else none. Late/worked/overtime/shortfall are derived at read time from real check-in/out against the plan (never stored). Facility wall clock offset = `FACILITY_UTC_OFFSET_MIN` (default 330, IST) because the server runs in UTC. A FIXED duty on the profile writes Mon–Sat, Sunday off. "Absent" only counts people who were scheduled — an owner/admin with no schedule is never forced into attendance. Self check-in/out now takes a camera photo; photos are served only through `/api/staff/attendance-photo/[logId]` (auth + tenant checked, never public) and are WebP/JPEG-compressed in the browser. History/timeline: `staff_history` + attendance + leave. New hospitals get shift templates and default vitals at Create Tenant.
